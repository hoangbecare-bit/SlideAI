...
import {
  ...
  buildSlidePlanPromptValues,
  presentationGenerationPromptTemplate,
  presentationSlidePlanPromptTemplate,
...
export async function POST(req: Request) {
  ...
  try {
    ...
    try {
      ...
      const planningInput = buildSlidePlanPromptValues({
        ...request,
        currentDate,
      });
      const planningStream = await planningChain.stream(planningInput);
      const slidePlanChunks: string[] = [];
        ...
        const chunkText =
          typeof chunk === "string"
            ...
            : typeof chunk?.content === "string"
              ...
              : "";
        if (chunkText) {
          slidePlanChunks.push(chunkText);
        }
      }

      slidePlan = slidePlanChunks.join("").trim();
      if (slidePlan.length === 0) {
        routeLogger.warn("Slide planning produced an empty plan", {
          requestId,
          title: request.title,
          totalSlides,
        });
      ...
      routeLogger.warn("Slide planning failed, continuing without slide plan", {
        requestId,
        error: planningError instanceof Error ? planningError.message : planningError,
        title: request.title,
        totalSlides,
      });
    }
