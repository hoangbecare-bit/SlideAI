...
import { type PresentationImageSearchResult } from "./image-search";
import { LAYOUT_REFERENCE } from "./layout-catalog";
...
Your task is to create exactly {TOTAL_SLIDES} slides. Use the outline for coverage and sequence, then write stronger slide copy when the outline wording is too raw. If a slide plan is supplied in the user context, use it as a storyboard to shape the final deck before generating XML. Match the requested language, tone, audience, scenario, and text density.
...
const SLIDE_PLAN_SYSTEM_PROMPT_TEMPLATE = `You are a presentation planning specialist. Create a concise slide-by-slide storyboard that helps a slide generator create a stronger deck.

Requirements:
- Return only a concise markdown plan.
- Cover all slides in order.
- For each slide, include a short objective, the key message, and the recommended visual style.
- Keep the plan practical and easy for downstream generation.
- Use the requested language and tone.
- Respect the requested text density, audience, scenario, and any visual or branding hints provided.
- Label each slide clearly with its slide number.
- Do not write XML or markdown headings for the slide descriptions; use simple numbered slide items.
`;

const SLIDE_PLAN_USER_PROMPT_TEMPLATE = `{USER_CONTEXT}`;

export const presentationSlidePlanPromptTemplate =
  ChatPromptTemplate.fromMessages([
    ["system", SLIDE_PLAN_SYSTEM_PROMPT_TEMPLATE],
    ["human", SLIDE_PLAN_USER_PROMPT_TEMPLATE],
  ]);
...
export function buildSlidePlanPromptValues(
  input: PresentationGenerationPromptInput,
): Record<string, string | number> {
  return {
    USER_CONTEXT: buildSlidePlanUserContext(input),
    TOTAL_SLIDES: input.outline.length,
  };
}
...
function interpolatePromptTemplate(
  ...
  return Object.entries(values).reduce(
    (renderedTemplate, [key, value]) =>
      renderedTemplate.split(`{${key}}`).join(String(value)),
    template,
  );
}
...
function buildUserContext(input: PresentationGenerationPromptInput): string {
  const sections = [
    ...
    formatSlidePlanContext(input.slidePlan),
  ].filter((section) => section.length > 0);

  return sections.join("\n\n");
}

function buildSlidePlanUserContext(input: PresentationGenerationPromptInput): string {
  const sections = [
    formatRequestContext(input),
    formatOutlineContext(input.outline),
  ].filter((section) => section.length > 0);
...
function formatSlidePlanContext(slidePlan?: string): string {
  if (!slidePlan || slidePlan.trim().length === 0) {
    return "";
  }
